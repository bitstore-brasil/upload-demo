...
final class Loader {
	...
	public function library($route) {
		...
		if (is_file($file)) {
			...
			throw new \Exception('Erro: Não foi possível carregar a library ' . $route . '!');
		}
	}
	...
	public function helper($route) {
		...
		if (is_file($file)) {
			...
			throw new \Exception('Erro: Não foi possível carregar o helper ' . $route . '!');
		}
	}
		...
		return function () use ($route) {
			...
			if ($result && !$result instanceof Exception) {
				...
				if (is_callable($callable)) {
					...
					throw new \Exception('Erro: Não foi possível chamar model/' . $route . '!');
				}
			}
